 ...
 * The purpose of this filter is to check if a user is a senior cashier, and if so, to validate their access to specific commands and pages.
 */
@WebFilter(urlPatterns = "/*")
public class SeniorCashierFilter implements Filter {

    final static Logger logger = Logger.getLogger(SeniorCashierFilter.class);
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (user != null && user.getRole().equals("senior cashier")) {
            if (path.startsWith("/login") || path.startsWith("/logout") || path.startsWith("/workbench")) filterChain.doFilter(servletRequest, servletResponse);
                ...
                if (command == null) {
                    ...
                    if (command.equals("SetActiveTab") || command.equals("ChangeLang")) filterChain.doFilter(servletRequest, servletResponse);
                        ...
                        if (!correctUrl)
                            for (String permission : RolesPermissions.seniorCashierPermissions)
                        ...
                        if (!correctUrl) {
                            NotificationManager.setWarningNotification(req, "Unable to: " + command + " with senior cashier role.");
                            logger.warn("Unable to: " + command + " with senior cashier role");
                            res.sendRedirect("/workbench");
