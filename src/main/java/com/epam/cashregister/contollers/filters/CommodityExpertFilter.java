 ...
 * The purpose of this filter is to check if a user is a commodity expert, and if so, to validate their access to specific commands and pages.
 */
@WebFilter(urlPatterns = "/*")
public class CommodityExpertFilter implements Filter {

    final static Logger logger = Logger.getLogger(CommodityExpertFilter.class);
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (user != null && user.getRole().equals("commodity expert")) {
            if (path.startsWith("/login") || path.startsWith("/logout") || path.startsWith("/workbench")) filterChain.doFilter(servletRequest, servletResponse);
                ...
                if (command == null) {
                    ...
                    if (command.equals("SetActiveTab") || command.equals("ChangeLang")) filterChain.doFilter(servletRequest, servletResponse);
                        ...
                        if (!correctUrl)
                            for (String permission : RolesPermissions.commodityExpertPermissions)
                        ...
                        if (!correctUrl) {
                            NotificationManager.setWarningNotification(req, "Unable to: " + command + " with commodity expert role.");
                            logger.warn("Unable to: " + command + " with commodity expert role");
                            res.sendRedirect("/workbench");
